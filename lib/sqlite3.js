...
['all', 'each', 'get'].forEach(function (name) {
    var allCallback = function(nestTables, callback) {
    ...
    var getCallback = function(nestTables, callback) {
        var cb = allCallback(nestTables, function(err, rows, fields) {
    ...
    var wrapCallback = function(name, nestTables, callback) {
        return name == 'all' ? allCallback(nestTables, callback) : getCallback(nestTables, callback);
    };
    ...
    Database.prototype[name] = function () {
        ...
        if (typeof sql == 'object') {
            if (sql.nestTables) nestTables = sql.nestTables;
            sql = sql.sql;
        }
        var callback = args.pop();
        args.unshift(sql);
        args.push(wrapCallback(name, nestTables, callback));
    ...
    Statement.prototype[name] = function () {
        var args = Array.prototype.slice.call(arguments);
        var callback = args.pop();
        args.push(callback.length < 3 ? wrapCallback(name, false, callback) : callback);
