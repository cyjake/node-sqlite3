...
Local<Array> Statement::GetFields(sqlite3_stmt* stmt) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        const char* orgName = sqlite3_column_origin_name(stmt, i);
        Nan::Set(field, Nan::New<String>("orgName").ToLocalChecked(), Nan::New<String>(orgName ? orgName : "").ToLocalChecked());
        const char* table = sqlite3_column_table_alias_name(stmt, i);
        Nan::Set(field, Nan::New<String>("table").ToLocalChecked(), Nan::New<String>(table ? table : "").ToLocalChecked());
        const char* orgTable = sqlite3_column_table_name(stmt, i);
        Nan::Set(field, Nan::New<String>("orgTable").ToLocalChecked(), Nan::New<String>(orgTable ? orgTable : "").ToLocalChecked());
